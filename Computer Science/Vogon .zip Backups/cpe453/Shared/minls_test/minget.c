...
int main(int argc, char **argv)
{
   ...
   int offset = 0;
   char *imagefile = NULL, *path = NULL, *dest = NULL, *read_path = NULL;
   char filename[60], temp_char;
   unsigned char signature[2] = {0};
   superblock_t superblock = {0};
   inode_t inode = {0};
   FILE *image = NULL, *outfile = NULL;
   ...
   while(strlen(read_path) > 0)
   {
      ...
      for(i=0;i<(inode.size)/(sizeof(directory_t));)
      {
         ...
         if(temp_dir.inode && (strcmp(temp_dir.name,filename) == 0))
         {/*seek to the inode it points to - 1*/
            print4("inode #: %lu, name:(%s),(%s)\n",
                  temp_dir.inode, temp_dir.name,filename);
   ...
   if((inode.mode & 0170000) == 0100000)
   {
      ...
      print2("now at block %ld.\n",ftell(image)/superblock.blocksize);
      /*open the dest if not null*/
      if(dest && (outfile = fopen(dest,"w")) == NULL)
      {
         perror("fopen 2");
         fclose(image);
         return -1;
      }
      print1("Past outfile opening\n");
      /*read and write bytes 1 at a time starting with first data
       * and ending after inode.size bytes.*/
      for(i=0;i<inode.size;i++)
      {
         if(fread(&temp_char,1,1,image) == -1)
         {
            perror("fread 17");
            fclose(image);
            return -1;
         }
         /*if no dest, write to stdout, else to opened stream*/
         if(!dest)
         {
            if(fwrite(&temp_char,1,1,stdout) == -1)
            {
               perror("fwrite 1");
               fclose(image);
               return -1;
         ...
         {
            if(fwrite(&temp_char,1,1,outfile) == -1)
            {
               perror("fwrite 2");
               fclose(image);
               return -1;
