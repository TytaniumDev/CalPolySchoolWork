...
public class ConvexPolygon implements Shape
{
   ...
   public void draw(java.awt.Graphics graphic)
   {
      ...
      for(int i = 0; i < vertices.length; i++)
      {
         xPoints[i] = this.vertices[i].x;
         yPoints[i] = this.vertices[i].y;
      }

    
      if(this.filled)
      {
         graphic.fillPolygon(xPoints, yPoints, vertices.length);
      }
      else
      {
         graphic.drawPolygon(xPoints, yPoints, vertices.length);  
      }
   }
}