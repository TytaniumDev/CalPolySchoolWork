 ...
 * $Log: pipeline.c,v $
 *
 * Revision 1.8  2010-03-11 15:31:32 by Tyler Holland (username: tyhollan)
...
extern void do_pipeline(pipeline pl)
{
  ...
  /* Make some pipes */
  pipefd = (int**)malloc(((pl->length) - 1) * sizeof(int*)); /* Number of pipes */
  ...
  for(i = 0; i < pl->length; i++) /* loop through all of the stages */
  {
    ...
    if(cpid[i] == 0)
    {
      ...
      /* 1 -> pipe -> 0 */
      /* so fd's are: */
      /* input: pipefd[i-1][0] , output: pipefd[i][1] */
