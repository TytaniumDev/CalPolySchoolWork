...
int main(int argc, char *argv[])
{
   ...
   int outputoccur = 0;
   char *outfile = NULL;
   int valstorage[STORAGE];
   char argstorage[STORAGE];
   ...
   do
   {
      ...
      while(rlen > 0) /*Still something in the file*/
      {
         for(i = 0; i < rlen; i++)
         {
            ...
            else /*It is whitespace*/
            {
               ...
               /*Parsing Logic*/
               else if(strncmp(holder, "P", 1) == 0) /*P value*/
               {
         ...
         while(rlen > 0) /*Still something in the file*/
         {
            for(; i < rlen; i++) /*No i initializer to keep buf space*/
            {
               ...
               else /*It is whitespace*/
               {
                  ...
                  /*Get any extra whitespace characters out of the way*/
                  while(isspace(buf[i + 1]) != 0)
                  {
                     i++;
                  }
      ...
      /*Manipulate image*/
      i = 0;
      while(argstorage[i] != '\0')
      {
         if(argstorage[i] == 'm')
         {
            maxval = maxvaledit(valstorage[i], readmaxval, array, height, width);
         }
         else if(argstorage[i] == 's')
         {
            for(j = 0; j < valstorage[i]; j++) /*Smooth j times*/
            {
               smoothfunc(array, height, width);
            }
         }
         else if(argstorage[i] == 'b')
         {
            maxval = brightenfunc(maxval, array, valstorage[i], height, width);
         }
         else if(argstorage[i] == 'd')
         {
            maxval = darkenfunc(maxval, array, valstorage[i], height, width);
         }
         i++;
      }
      if(maxval == -1)
      ...
      if(outputoccur > 0)
      {
         if(bigloop != 0)
         {  
            *appendfile = (char)bigloop;
            appendfile = strcat(appendfile, outfile);
            outfile = appendfile;
         }
