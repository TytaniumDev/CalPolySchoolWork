...
(define (changebox! box val)
  (sdo (st <- getstore)
       (dc <- (hash-set (sto-memory st) (boxV-loc box) val))
       (lift boolV #f)))
  ...
  (type-case OCFAE expr
    ...
    [withrec (name rhs body)
             (begin
               ...
               (sdo (closure <- (interp rhs new-env))
                    (lift (set-box! new-box closure))
                    (interp body new-env)))]
    [app (f args)
