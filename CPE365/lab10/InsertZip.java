...
public class InsertZip
{
   public static void main (String[] args ){
      ...
      try {
         ...
         while (zip_.length( ) != 0){  
            System.out.println
               ("Enter ZIP then CITY, or press enter to quit");
            zip_ = keyboard.nextLine( );
            try {
               // Set the parameter in the query string 
               // prepared with pstmt1
               city_ = zip_.substring(6); //Because zips are always 5 long
               zip_ = zip_.substring(0,5);
               pstmt1.setString(1, zip_);  
